...
<<<<<<< HEAD
            ...
            data[['Code', 'Open', 'Low', 'High', 'Close', 'Adj_close', 'Date']] = data[[
                'symbol', 'open', 'low', 'high', 'close', 'adjClose', 'date']]
            Quantity_date = data[['Code', 'Open', 'Low',
                                  'High', 'Close', 'Adj_close', 'Date']]

            Quantity_date['Date'] = pd.to_datetime(
                Quantity_date['Date']).map(lambda x: x.date())
            ...
            result = pd.concat(
                [code, date, open, low, high, close, Adj_close], axis=1, ignore_index=True)

            result.columns = ['Code', 'Date', 'Open',
                              'Low', 'High', 'Close', 'Adj_close']

            Quantity_date = Quantity_date.drop(
                ['Code', 'Date', 'Open', 'Low', 'High', 'Close'], axis=1)
...
<<<<<<< HEAD
            ...
            pre.Date[-8:] = idx.map(lambda x: x.date()
...
<<<<<<< HEAD
            ...
            print("Tomorrow's", quote,
                  " Closing Price Prediction by ARIMA:", tomorrow_ar)
...
<<<<<<< HEAD
        ...
        model.fit(X_train, y_train, validation_data=(
            X_test, ytest), epochs=10, batch_size=100, verbose=1)
...
<<<<<<< HEAD
        ...
        tweets = tweepy.Cursor(user.search, q=str(
            symbol), tweet_mode='extended', lang='en', exclude_replies=True).items(ct.num_of_tweets)
        ...
        pie['labels'] = labels
        big_data = {"sizes": np.array(
            pie.sizes), "labels": np.array(pie.labels)}
        df2 = pd.DataFrame(big_data)
        k = df2.to_dict('records')
        out_file = open("static/assets/js/dashboard/pie.json",
                        "w", encoding='utf-8')
    ...
    def recommending(df, global_polarity, today_stock, mean):
        if today_stock.iloc[-1]['adjClose'] < mean:
            if global_polarity > 0:
                ...
                print(
                    "##############################################################################")
                print("According to the DL Predictions and Sentiment Analysis of Tweets, a",
...
<<<<<<< HEAD
        ...
        forecast_set_ar = np.round(
            np.array(all_pred["ARIMA"].tail(7)), 2).reshape(-1, 1)
        ...
        k = df2.to_dict('records')
        out_file = open("static/assets/js/dashboard/trends.json",
                        "w", encoding='utf-8')
        simplejson.dump(k, out_file, ignore_nan=True,
                        ensure_ascii=False, indent=4)

        big_data = {"Date": np.array(
            df3.Date), "ARIMA": np.array(pre["ARIMA"])}
        df2 = pd.DataFrame(big_data)
        ...
        k = df2.to_dict('records')
        out_file = open(
            "static/assets/js/dashboard/pastpreds.json", "w", encoding='utf-8')
        simplejson.dump(k, out_file, ignore_nan=True,
                        ensure_ascii=False, indent=4)
        ...
        k = df2.to_dict('records')
        out_file = open(
            "static/assets/js/dashboard/forecast.json", "w", encoding='utf-8')
        simplejson.dump(k, out_file, ignore_nan=True,
                        ensure_ascii=False, indent=4)

        return render_template('results.html', quote=quote, arima_pred=round(tomorrow_ar, 2),
                               open_s=today_stock['open'].to_string(
                                   index=False),
...
def QL(df):
        ...
        def train(self, iterations, checkpoint, initial_money):
            for i in range(iterations):
                ...
                for t in range(0, len(self.trend) - 1, self.skip):
                    ...
                    self._memorize(state, action, invest,
    ...
    agent_test.train(iterations=200, checkpoint=10,
